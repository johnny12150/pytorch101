...
test_x = Variable(torch.unsqueeze(testing_data.test_data, dim=1).float(), requires_grad=False).view(-1, 28*28).to(Device)
...
for e in range(20):
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # y = Variable(target, requires_grad=False).to(Device)
        classifier.train()
        optimizer.zero_grad()
        pred_y = classifier(x)
